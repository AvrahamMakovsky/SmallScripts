...
Remote Plug and Play device search utility for Windows hosts (Python + PowerShell Remoting).
...
- Opens Notepad so a host list can be pasted quickly during daily work.
...
You need to quickly check whether a specific USB adapter, serial adapter, network device, lab dongle, or other Device Manager item is currently detected on a list of remote lab machines.
...
This is a focused lab/support utility, not a replacement for full inventory platforms such as SCCM, Intune, Lansweeper, or PDQ Inventory.

Use it only on systems you are authorized to query. For large managed environments, a central inventory platform is usually more appropriate. For quick checks across a limited host list, this script is intended to stay simple and fast.
